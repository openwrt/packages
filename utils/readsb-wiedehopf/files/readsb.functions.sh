...
# don't always have a resolver. The inode-ownership check limits the
# result to readsb; multiple connectors on one port return indeterminate.
#
# Returns:
...
readsb_connector_active() {
	...
	# A port-only kernel match is ambiguous when several configured
	# connectors share that port. Let the caller probe the exact host.
	local connector_count
	connector_count=$(readsb_cmdline | tr ' ' '\n' \
		| awk -F, -v p="$port" '$1 ~ /^--net-connector=/ && $2 == p { n++ } END { print n+0 }')
	[ "$connector_count" -le 1 ] || return 2
