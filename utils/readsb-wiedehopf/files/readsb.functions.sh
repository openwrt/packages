...
_readsb_emit_serial() {
	...
	if [ -r "$d/serial" ]; then
		IFS= read -r s < "$d/serial"
		s=$(readsb_sanitize_serial "$s")
		[ -n "$s" ] && ser=$s
	fi
	echo "$ser"
}

# 0 when an attached SDR has exactly this sanitized serial.
readsb_sdr_serial_present() {
	readsb_list_sdrs | grep -Fqx -e "$1"
}
...
# Strip control bytes from a librtlsdr serial while preserving printable identity.
readsb_sanitize_serial() {
	printf '%s' "$1" | LC_ALL=C tr -d '\000-\037\177'
}

# 0 when a numeric librtlsdr device index exists for the current SDR count.
readsb_device_index_valid() {
	awk -v d="$1" -v count="$2" 'BEGIN {
		exit !(d ~ /^[0-9]+$/ && d+0 >= 0 && d+0 < count)
	}'
}
...
# 0 only if `readsb.<name>` exists and is a feeder section.
readsb_feeder_section_exists() {
	[ "$(uci -q get "readsb.$1")" = feeder ]
}
...
#
# The inode-ownership check limits the result to readsb. A single connector
# uses a remote-port match so DNS rotation cannot hide a live socket. When
# connectors share a port, resolve the requested host and also match the
# remote IPv4 address so each socket can be attributed to its feeder.
#
# Returns:
...
readsb_connector_active() {
	...
	local connector_count remote_ips=''
	connector_count=$(readsb_cmdline | tr ' ' '\n' \
		| awk -F, -v p="$port" '$1 ~ /^--net-connector=/ && $2 == p { n++ } END { print n+0 }')
	if [ "$connector_count" -gt 1 ]; then
		command -v resolveip >/dev/null 2>&1 || return 2
		remote_ips=$(resolveip -4 -t 3 "$host" 2>/dev/null | awk -F. '
			NF == 4 {
				for (i = 1; i <= 4; i++)
					if ($i !~ /^[0-9]+$/ || $i + 0 > 255) next
				printf "%02X%02X%02X%02X ", $4, $3, $2, $1
			}
		')
		[ -n "$remote_ips" ] || return 2
	fi

	# /proc/net/tcp uses uppercase hex for ports. awk normalizes values
	# such as 080 as decimal rather than letting printf treat them as octal.
	local hexport
	hexport=$(awk -v p="$port" 'BEGIN {
		if (p !~ /^[0-9]+$/ || p+0 < 1 || p+0 > 65535) exit 1
		printf "%04X", p+0
	}') || return 2
	...
	# whose remote endpoint matches and whose inode is owned by readsb.
	# remote_ips holds resolved IPv4 (tcp) addresses only, so tcp6 rows
	# and any row whose address fell outside the just-resolved set (DNS
	# rotation since readsb connected) can't be confirmed -- but they
	# also can't be ruled out, so they mark "other" (rc 2) rather than
	# being silently dropped into a false "not connected" (rc 1).
	awk -v hp="$hexport" -v inodes=" $inodes " -v remote_ips="$remote_ips" '
		FNR == 1 { next }
		{
			n = split($3, ra, ":")
			if (n != 2 || ra[2] != hp) next
			if ($4 != "01") next
			if (index(inodes, " " $10 " ") == 0) next
			if (remote_ips == "" ||
			    index(" " remote_ips " ", " " ra[1] " ") > 0) { found = 1; exit }
			other = 1
		}
		END { exit found ? 0 : (other ? 2 : 1) }
