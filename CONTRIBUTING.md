...
Patches live in `<package>/patches/`, are applied with quilt, and are expected
...
Every patch starts with a `git am` compatible header, so that it can be
applied to an upstream tree without editing. All four lines are required and
their absence is a hard failure in the formality check:
