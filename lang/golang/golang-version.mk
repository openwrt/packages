...
# $(1) bootstrap version id
# $(2) go version id
# $(3) go version patch level (optional)
# $(4) hash
define Go/AddVersion
  BOOTSTRAP_VERSION_ID:=$(1)
  GO_VERSION_MAJOR_MINOR:=$(2)
  GO_VERSION_PATCH:=$(3)
  PKG_HASH:=$(4)

  PKG_NAME:=golang$$(GO_VERSION_MAJOR_MINOR)
  ...
  PKG_MAINTAINER:=Jeffery To <jeffery.to@gmail.com>, George Sapkin <george@sapk.in>
  ...
  HOST_BUILD_DEPENDS:=golang$$(if $$(filter bootstrap,$$(BOOTSTRAP_VERSION_ID)),-)$$(BOOTSTRAP_VERSION_ID)/host
  HOST_BUILD_DIR:=$$(BUILD_DIR_HOST)/go-$$(PKG_VERSION)
  HOST_BUILD_PARALLEL:=1

  HOST_GO_PREFIX:=$$(STAGING_DIR_HOSTPKG)
  HOST_GO_VERSION_ID:=$$(GO_VERSION_MAJOR_MINOR)
  HOST_GO_PROFILE_ID:=Host-$$(HOST_GO_VERSION_ID)
  HOST_GO_ROOT:=$$(HOST_GO_PREFIX)/lib/go-$$(HOST_GO_VERSION_ID)
  BOOTSTRAP_DIR:=$$(HOST_GO_PREFIX)/lib/go-$$(BOOTSTRAP_VERSION_ID)
  ...
  # use the host std lib, so remove it now and force go to rebuild std for
  # target later
  define Host/Install
	$$(call Host/Uninstall)
