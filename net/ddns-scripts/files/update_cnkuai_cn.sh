...
getPixel(){
  ...
  echo `hexdump -s $(($x+$width*$y)) -n 3 -e '3/1 "%02X"' $filename`
}
...
local domainline=`awk "/<td>$domain<\/td>/{ print NR; exit }" /tmp/cnkuai.html`
local domainid=`awk "NR==$(($domainline+3))" /tmp/cnkuai.html | sed 's/^.*name=\x27domainid\x27 value="//g' | sed 's/".*$//g'`
local dnslistid=`awk "NR==$(($domainline+3))" /tmp/cnkuai.html | sed 's/^.*name=\x27dnslistid\x27 value="//g' | sed 's/".*$//g'`
