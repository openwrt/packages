#!/bin/sh
# Wrapper for acme/uacme to work on openwrt.
#
    ...
    for listener in $(get_listeners); do
        ...
        case "$cmd" in
            ...
            "")
                ...
                debug "Standalone mode not supported, setup uhttpd or nginx"
                return 1
            ;;
            *)
                err "$main_domain: unsupported (apache/haproxy?) daemon is listening on port 80."
                err "if webroot is set on your current webserver comment line 142 (return 1) from this script."
                return 1
            ;;
        esac
    done
...
issue_cert()
{
    ...
    if [ "$APP" = "uacme" ]; then
       [ "$DEBUG" -eq "1" ] && acme_args="--verbose --verbose"
    elif [ "$APP" = "acme" ]; then
    ...
    if [ "$APP" = "uacme" ]; then
       if [ ! -f  "${domain_dir}/private/key.pem" ]; then
           log "Create a new ACME account with email $ACCOUNT_EMAIL use staging=$use_staging"
           run_acme $acme_args --confdir "{domain_dir}" $staging --yes new $ACCOUNT_EMAIL
       fi

       if [ -f "${domain_dir}/$main_domain/cert.pem" ]; then
           log "Found previous cert config, use staging=$use_staging. Issuing renew."
           [ "$keylength_ecc" -eq "1" ] && acme_args="$acme_args --type=EC"
           export CHALLENGE_PATH="$webroot"
           run_acme $acme_args --confdir "{domain_dir}" $staging --never-create issue $domains --hook=$HPROGRAM && ret=0 || ret=1
           post_checks
           return $ret
       fi
    fi
...
#          uacme_args="$uacme_args --dns $dns"
    ...
    if ! run_acme $workdir "$STATE_DIR" $staging --issue $acme_args $HOOK; then
    ...
    if [ -e /etc/init.d/uhttpd ] && [ "$update_uhttpd" -eq "1" ]; then
        if [ "$APP" = "uacme" ]; then
            uci set uhttpd.main.key="${domain_dir}/private/${main_domain}.key"
    ...
    if [ -e /etc/init.d/nginx ] && [ "$update_nginx" -eq "1" ]; then
        ...
        sed -i "s#ssl_certificate_key\ .*#ssl_certificate_key ${domain_dir}/private/${main_domain}.key;#g" /etc/nginx/nginx.conf
        else
        sed -i "s#ssl_certificate\ .*#ssl_certificate ${domain_dir}/fullchain.cer;#g" /etc/nginx/nginx.conf
    ...
    if [ -e /etc/init.d/haproxy ] && [ "$update_haproxy" -eq 1 ]; then
        if [ "$APP" = "uacme" ]; then
            cat ${domain_dir}/${main_domain}/cert.pem ${domain_dir}/private/${main_domain}/key.pem > ${domain_dir}/${main_domain}/full_haproxy.$
        else
            cat ${domain_dir}/${main_domain}/fullchain.cer ${domain_dir}/${main_domain}/${main_domain}.key > ${domain_dir}/${main_domain}/full_$
        fi
    fi
