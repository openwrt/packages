...
m = Map("clamav", translate("Anti-Virus"))
...
s:tab("tab_advanced", translate("General Settings"))
...
for k, v in pairs(conn:call("service", "list", { name="clamav" })) do
        status="running"
end
...
button.write = function(self, section)
        if status == "not running" then
                sys.call("/etc/init.d/clamav start >/dev/null")
                button.inputtitle = "ON"
                button.title = "Status: "
        else
                sys.call("/etc/init.d/clamav stop >/dev/null")
                button.inputtitle = "OFF"
                button.title = "Status: "
        end
end

dummy1 = s:taboption("tab_basic", DummyValue, "")
dummy2 = s:taboption("tab_basic", DummyValue, "")


database_status = s:taboption("tab_basic", TextValue, "lines", "")
database_status.wrap = "off"
database_status.rows = 10
database_status.rmempty = true

function database_status.cfgvalue()
        local uci = require "luci.model.uci".cursor_state()
                sys.call("echo \"================== Database status ==================\n\" > /tmp/db_status.log")
                sys.call("grep -e update -e \"up to date\" -e signatures /tmp/freshclam.log >> /tmp/db_status.log")
                local file = "/tmp/db_status.log"
                if file then
                        return fs.readfile(file) or ""
                else
                        return "Can't read db_status.log"
                end
end
...
if line == "yes" then
	...
	MaxFileSize:value("25M",translate("25M"))
	MaxFileSize:value("30M",translate("30M"))
	MaxFileSize.default = "25M"
