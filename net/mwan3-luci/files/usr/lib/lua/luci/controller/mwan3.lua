...
function getInterfaceStatus(ruleNumber, interfaceName)
	if ut.trim(sys.exec("uci -p /var/state get mwan3." .. interfaceName .. ".enabled")) == "1" then
		if ut.trim(sys.exec("/usr/bin/ip route list table " .. ruleNumber)) ~= "" then
	...
	if tool == "service" then
		...
		if interfaceDevice ~= "" then
			...
			elseif tool == "rulechk" then
				getInterfaceNumber()
				local rule1 = sys.exec("/usr/bin/ip rule | grep $(echo $((" .. interfaceNumber .. " + 1000)))")
				local rule2 = sys.exec("/usr/bin/ip rule | grep $(echo $((" .. interfaceNumber .. " + 2000)))")
			...
			elseif tool == "routechk" then
				getInterfaceNumber()
				local routeTable = sys.exec("/usr/bin/ip route list table " .. interfaceNumber)
				if routeTable ~= "" then
...
function troubleshootingData()
	...
	-- ip rule show
	local ipRuleShow = ut.trim(sys.exec("/usr/bin/ip rule show"))
		if ipRuleShow == "" then
	...
	local routeList, routeString = ut.trim(sys.exec("/usr/bin/ip rule | sed 's/://g' | awk '$1>=2001 && $1<=2250' | awk '{print $NF}'")), ""
		if routeList ~= "" then
			for line in routeList:gmatch("[^\r\n]+") do
				routeString = routeString .. line .. "\n" .. sys.exec("/usr/bin/ip route list table " .. line)
			end
