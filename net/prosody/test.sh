...
prosody)
	...
	# prosodyctl drops to the prosody user when it is started as root, but
	# the data directory ships root-owned; the init script chowns it at
	# service start, so mirror that here or every storage write is denied.
	# Doubles as a check that USERID actually created the prosody user.
