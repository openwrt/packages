...
manage_sa() {
  ...
  for litem in $2 ; do
    for ritem in $3 ; do
      echo "
spd$spdcmd $litem $ritem any -P out ipsec esp/tunnel/$gate-$4/unique;
spd$spdcmd $ritem $litem any -P in ipsec esp/tunnel/$4-$gate/unique;
" | /usr/sbin/setkey -c 1>&2
    done
  done

  for ritem in $3 ; do
    (sleep 3; /usr/sbin/ip route $rtcmd $ritem via $data) &
  done
}
