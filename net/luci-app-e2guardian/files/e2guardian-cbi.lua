...
m = Map("e2guardian", translate("Web Filter"))
...
for k, v in pairs(conn:call("service", "list", { name="e2guardian" })) do
        status="running"
end
...
button.write = function(self, section)
        if status == "not running" then
                sys.call("/etc/init.d/e2guardian start >/dev/null")
                button.inputtitle = "ON"
                button.title = "Status: "
        else
                sys.call("/etc/init.d/e2guardian stop >/dev/null")
                button.inputtitle = "OFF"
                button.title = "Status: "
        end
end
...
local dummy1, drugs, porn, sexuality, weapons

dummy1 = s:taboption("tab_basic", DummyValue, "dummy1", translate("Content filtering:"))

drugs = s:taboption("tab_basic", Flag, "drugs", translate("Drugs"))
drugs.default=drugs.disabled

porn = s:taboption("tab_basic", Flag, "porn", translate("Porn"))
porn.default=porn.disabled

sexuality = s:taboption("tab_basic", Flag, "sexuality", translate("Sexuality"))
sexuality.default=sexuality.disabled

weapons = s:taboption("tab_basic", Flag, "weapons", translate("Weapons"))
weapons.default=weapons.disabled

sys.call("head -n 50 /etc/e2guardian/lists/bannedurllist > /tmp/tmpfile && mv /tmp/tmpfile /etc/e2guardian/lists/bannedurllist")
sys.call("head -n 115 /etc/e2guardian/lists/bannedsitelist > /tmp/tmpfile && mv /tmp/tmpfile /etc/e2guardian/lists/bannedsitelist")
function drugs.write(self, section, value)
	sys.call("echo '.Include</etc/e2guardian/lists/blacklists/drugs/urls>' >> /etc/e2guardian/lists/bannedurllist")
	sys.call("echo '.Include</etc/e2guardian/lists/blacklists/drugs/domains>' >> /etc/e2guardian/lists/bannedsitelist")
	drugs.default=drugs.enabled
end
function porn.write(self, section, value)
        sys.call("echo '.Include</etc/e2guardian/lists/blacklists/porn/urls>' >> /etc/e2guardian/lists/bannedurllist")
	sys.call("echo '.Include</etc/e2guardian/lists/blacklists/porn/domains>' >> /etc/e2guardian/lists/bannedsitelist")
        porn.default=porn.enabled
end
function sexuality.write(self, section, value)
        sys.call("echo '.Include</etc/e2guardian/lists/blacklists/sexuality/urls>' >> /etc/e2guardian/lists/bannedurllist")
	sys.call("echo '.Include</etc/e2guardian/lists/blacklists/sexuality/domains>' >> /etc/e2guardian/lists/bannedsitelist")
        sexuality.default=sexuality.enabled
end
function weapons.write(self, section, value)
        sys.call("echo '.Include</etc/e2guardian/lists/blacklists/weapons/urls>' >> /etc/e2guardian/lists/bannedurllist")
	sys.call("echo '.Include</etc/e2guardian/lists/blacklists/weapons/domains>' >> /etc/e2guardian/lists/bannedsitelist")
        weapons.default=weapons.enabled
end
...
if line == "yes" then
	...
	logadblock = s:taboption("tab_general", ListValue, "logadblock", translate("Enable loging of ADs"))
	logadblock:value("on",  translate("Yes"))
	logadblock:value("off", translate("No"))
	logadblock.default = "off"

	loguseragent = s:taboption("tab_general", ListValue, "loguseragent", translate("Enable loggin of client user agent"))
