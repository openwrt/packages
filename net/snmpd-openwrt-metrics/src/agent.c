 ...
 *
 * Three subtrees are served:
 *   .1.3.6.1.4.1.66510.1.1    OPENWRT-MIB device metadata (OpenWrt IANA PEN 66510)
 *   .1.3.6.1.4.1.66510.1.10   OPENWRT-WIRELESS-MIB
...
static oid sys_scalar_oid[]     = { 1,3,6,1,4,1,66510,1,1,0 }; /* last arc set per field */
...
/* Device metadata, indexed by enum sys_field. Only fields the device
 * provides are registered, so an absent one is simply not instantiated. */
static char sys_values[SYS_FIELD_MAX + 1][SYS_VALUE_MAX];

static int sys_handler(netsnmp_mib_handler *handler,
		       netsnmp_handler_registration *reginfo,
		       netsnmp_agent_request_info *reqinfo,
		       netsnmp_request_info *requests)
{
	const char *v = sys_values[reginfo->rootoid[9]];
	...
	snmp_set_var_typed_value(requests->requestvb, ASN_OCTET_STR,
				 (const u_char *)v, strlen(v));
	return SNMP_ERR_NOERROR;
}

static void register_sysinfo(void)
{
	static const char *const names[SYS_FIELD_MAX + 1] = {
		[SYS_VERSION]  = "openwrtSysVersion",
		[SYS_REVISION] = "openwrtSysRevision",
		[SYS_TARGET]   = "openwrtSysTarget",
		[SYS_MODEL]    = "openwrtSysModel",
	};
	int f;

	for (f = 1; f <= SYS_FIELD_MAX; f++) {
		if (!sysinfo_read(f, sys_values[f], sizeof(sys_values[f])))
			continue;
		/* netsnmp copies the OID into the registration */
		sys_scalar_oid[9] = f;
		netsnmp_register_read_only_scalar(
			netsnmp_create_handler_registration(
				names[f], sys_handler, sys_scalar_oid,
				OID_LENGTH(sys_scalar_oid), HANDLER_CAN_RONLY));
	}
}
