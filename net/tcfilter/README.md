...
* A `procd_add_reload_trigger` reloads the service when the `tcfilter`
  config changes, so LuCI Save & Apply and
  `uci commit tcfilter && reload_config` take effect on their own; a bare
  `uci commit` still needs an explicit `/etc/init.d/tcfilter reload`.
