...
JOOL_DEFAULT_VETH_HOST_V4="192.0.2.0"
JOOL_DEFAULT_VETH_NS_V4="192.0.2.1"
...
jool_clat_proto_setup_instance() {
	...
	# Prepopulate the namespace-side neighbor entry to avoid relying on ARP
	# and NDP, which may require extra firewall rules to work.
	# The host-side neighbor entries are populated later via proto_add_ipv[46]_neighbor.
	jool_clat_proto_run "$cfg" "failed to populate namespace IPv6 neighbor entry for '$veth_host_v6'" \
		ip netns exec "$netns" ip -6 neigh replace "$veth_host_v6" lladdr "$veth_host_mac" dev "$veth_ns_ifname" nud permanent || return 1
	jool_clat_proto_run "$cfg" "failed to populate namespace IPv4 neighbor entry for '$veth_host_v4'" \
		ip netns exec "$netns" ip -4 neigh replace "$veth_host_v4" lladdr "$veth_host_mac" dev "$veth_ns_ifname" nud permanent || return 1
	jool_clat_proto_run "$cfg" "failed to assign '$veth_ns_v4' peer '$veth_host_v4' on '$veth_ns_ifname'" \
		ip netns exec "$netns" ip addr add "$veth_ns_v4" peer "$veth_host_v4" dev "$veth_ns_ifname" || return 1
	...
	# Prevent Jool SIIT from capturing INPUT traffic toward the ns veth itself
	# (e.g. ping for test).
	jool_clat_proto_run "$cfg" "failed to add denylist" \
		ip netns exec "$netns" jool_siit denylist4 add "${veth_ns_v4}" || return 1
...
proto_jool_clat_setup() {
	...
	local prefix_count4 prefix_count6 prefix6
	...
	proto_add_ipv4_address "$veth_host_v4" "" "" "$veth_ns_v4"
	[ "$defaultroute" = 0 ] || proto_add_ipv4_route "0.0.0.0" 0 "$veth_ns_v4" "" "$metric"
	proto_add_ipv4_neighbor "$veth_ns_v4" "$JOOL_CLAT_VETH_NS_MAC"
...
proto_jool_clat_teardown() {
	local cfg="$1"
	local veth_host_ifname netns

	json_get_vars veth_host_ifname netns
	veth_host_ifname="${veth_host_ifname:-veth-$cfg}"
	netns="${netns:-ns-$cfg}"
