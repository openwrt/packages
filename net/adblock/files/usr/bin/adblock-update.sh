...
then
    ...
    /usr/bin/logger -t "adblock[${pid}]" "error: adblock configuration not found"
...
then
    ...
    then
        ...
        print "$(/bin/date "+%d.%m.%Y %H:%M:%S") - error: shallalist archive download failed (${shalla_url})" >> "${log_file}"
    ...
    then
        ...
        print "$(/bin/date "+%d.%m.%Y %H:%M:%S") - error: shallalist archive extraction failed" >> "${log_file}"
    ...
    then
        ...
        print "$(/bin/date "+%d.%m.%Y %H:%M:%S") - error: shallalist category merge failed (${rc}, ${shalla_cat})" >> "${log_file}"
...
# prepare output and store all domains in temp file
#
for src in ${adb_source}
do
    url="${src//\&ruleset=*/}"
    eval "ruleset=\$rset_${src//*\&ruleset=/}"
    if [[ "${url}" == @(https://*) ]]
    then
    ...
    if (( rc == 0 )) && [[ -n "${tmp_var}" ]]
    then
        adb_count=$(print "${tmp_var}" | eval "${ruleset}" | tee -a "${tmp_file}" | wc -l)
    ...
    then
        ...
        print "$(/bin/date "+%d.%m.%Y %H:%M:%S") - error: source download failed (${url})" >> "${log_file}"
